...
class ConnectionController extends ChangeNotifier {
  ...
  bool get _resolvedIsOffline =>
      isOffline || (mode == ConnectionMode.none && ble.isConnected);

  ConnectionController({required this.ble, required this.fb, ApiService? api})
      : _api = api {
  ...
  Future<void> checkBackendHealth() async {
    ...
    if (_lastHealthCheck != null &&
        now.difference(_lastHealthCheck!).inSeconds < 10) {
  ...
  Future<void> startSession(
      String crop, double targetTemp, double weight) async {
    ...
    if (isOnline) {
      ...
      if (api != null && backendOnline) {
        try {
          ...
          try {
            await fb.startSession(crop, targetTemp);
          } catch (_) {}
        } catch (e) {
          debugPrint(
              "Backend Start Session Error: $e. Falling back to Firebase.");
          try {
            await fb.startSession(crop, targetTemp);
          } catch (_) {}
        }
  ...
  Future<List<dynamic>> fetchMySessions() async {
    ...
    try {
      return await _api!
          .fetchMyRecords(); // Assuming this is the equivalent for now
    } catch (e) {
      return [];
    }
  }
  ...
  Future<void> setFanSpeed(int pct) async {
    if (isOnline) {
      if (api != null && backendOnline) {
        try {
          await api!
              .updateFanSpeed(fb.deviceId ?? MockData.defaultDeviceId, pct);
          try {
  ...
  Future<void> setTargetTemp(double temp) async {
    if (isOnline) {
      if (api != null && backendOnline) {
        try {
          await api!
              .updateTemperature(fb.deviceId ?? MockData.defaultDeviceId, temp);
          try {
  ...
  Future<void> tare() async {
    if (isOnline) {
      try {
        ...
        debugPrint("Firebase Tare Error: $e");
      }
    } else if (_resolvedIsOffline) {
      await ble.sendTare();
    }
