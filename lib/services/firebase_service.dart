...
class FirebaseDeviceService extends ChangeNotifier {
  ...
  Stream<Map<String, dynamic>> listenToLiveMetrics(String deviceId) {
    return _db.child('devices').child(deviceId).child('live').onValue.map((event) {
      if (event.snapshot.value != null) {
  ...
  Stream<Map<String, dynamic>> streamLiveState(String deviceId) {
    return listenToLiveMetrics(deviceId);
  }
