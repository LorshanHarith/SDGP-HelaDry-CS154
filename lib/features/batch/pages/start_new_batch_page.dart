...
class _StartNewBatchPageState extends State<StartNewBatchPage> {
  ...
  Future<void> _handleStartBatch() async {
    ...
    if (registeredDeviceId == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: Text('No device paired. Please pair your device first.')),
      );
      return;
    }
    ...
    final batchData = {
      ...
      "duration":
          int.tryParse(_durationController.text) ?? selectedCrop.durationHours,
    ...
    try {
      ...
      final url = Uri.parse('${AppConfig.baseUrl}/device/start');
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('New Drying Batch',
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              children: [
                Expanded(
                    child: _buildField('Weight (kg)', '0.0', _weightController,
                        isDark, cardColor,
                        keyboard: TextInputType.number)),
                const SizedBox(width: 15),
                Expanded(
                    child: _buildField(
                        'Trays Used', '1', _traysController, isDark, cardColor,
                        keyboard: TextInputType.number)),
              ],
            ),
            if (!_isAutoMode) ...[
              const SizedBox(height: 20),
              _buildField('Batch Name', 'Enter batch name',
                  _batchNameController, isDark, cardColor),
            ],
            ...
            Slider(
              ...
              onChanged:
                  _isAutoMode ? null : (v) => setState(() => _targetTemp = v),
            ),
            const SizedBox(height: 15),
            _buildField('Duration (Hours)', 'Time', _durationController, isDark,
                cardColor,
                enabled: !_isAutoMode, keyboard: TextInputType.number),
            const SizedBox(height: 20),
            _buildField('Batch Name (Optional)', 'Leave blank to use crop name',
                _batchNameController, isDark, cardColor),
  ...
  Widget _buildModeTab(String label, bool isActive, bool isDark,
  ...
  Widget _buildField(String label, String hint,
      TextEditingController controller, bool isDark, Color cardColor,
      {bool enabled = true, TextInputType keyboard = TextInputType.text}) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(label,
