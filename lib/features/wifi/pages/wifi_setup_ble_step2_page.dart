    ...
    try {
      ...
      if (deviceId.isNotEmpty) {
        if (userId != null) {
          ...
          wifiService
              .fetchNetworksFromFirebase(userId)
              .timeout(const Duration(seconds: 2))
              .catchError((_) => <SavedNetwork>[]);
        }
        await session
            .loadSavedNetworks(deviceId)
            .timeout(const Duration(seconds: 2))
            .catchError((_) {});
      }
    } catch (e) {
    ...
    if (!bleService.isConnected) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
                'Device not connected via Bluetooth. Go back and reconnect.'),
                      ...
                      children: [
                        if (_isLoadingNetworks)
                          ...
                          ...context
                              ...
                              .fullSavedNetworks
                              .map(
                                  (net) => _buildSavedNetworkItem(net, isDark)),
                        ...
                        if ((_state == 'results' || _state == 'scanning') &&
                            _showScannedNetworks) ...[
                          AnimatedSize(
