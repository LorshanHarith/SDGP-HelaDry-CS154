...
class _MyRecordsPageState extends State<MyRecordsPage> {
  ...
  Future<void> _fetchRecords() async {
    ...
    try {
      ...
      final response = await http.get(
        Uri.parse('${AppConfig.baseUrl}/session/my-sessions'),
        headers: {
  ...
  Widget _buildRecordItem(
      Map<String, dynamic> record, bool isDark, SessionStore session) {
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  '${record['weight_kg'] ?? 0}kg  •  ${record['trays'] ?? 0} trays  •  ${record['target_temperature'] == null ? '0°C' : session.useCelsius ? '${record['target_temperature']}°C' : '${((record['target_temperature'] is num ? record['target_temperature'].toDouble() : double.tryParse(record['target_temperature'].toString()) ?? 0.0) * 9 / 5 + 32).round()}°F'}',
                  style: TextStyle(
                    fontSize: 12,
                    color: isDark
                        ? const Color(0xFF8892B0)
